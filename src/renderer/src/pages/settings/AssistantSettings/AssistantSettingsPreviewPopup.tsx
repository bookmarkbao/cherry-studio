...
const PopupContainer: React.FC<Props> = ({ assistant, resolve }) => {
  ...
  const tabItems = [
    ...
    // 仅在 promptVisible 不为 false 时显示 prompt 标签页
    ...(assistant.settings?.promptVisible !== false
      ? [
          {
            key: 'prompt',
  ...
  // 如果只有一个标签页，直接显示内容，不显示 Tabs
  const shouldShowTabs = tabItems.length > 1
  ...
  return (
    <Modal
      ...
      centered>
      <Container>
        {shouldShowTabs ? <Tabs defaultActiveKey="parameters" items={tabItems} /> : parametersContent}
      </Container>
    </Modal>
  )
}
