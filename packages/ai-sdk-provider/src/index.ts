...
export const DEFAULT_CHERRYIN_GEMINI_BASE_URL = 'https://open.cherryin.net/gemini/v1beta'
...
const isAnthropicModel = (modelId: string) => {
  console.log('isAnthropicModel', modelId)
  return ANTHROPIC_PREFIX.test(modelId)
}
const isGeminiModel = (modelId: string) =>
  GEMINI_PREFIX.test(modelId) && !GEMINI_EXCLUDED_SUFFIXES.some((suffix) => modelId.endsWith(suffix))
