...
function CustomizedSwitch({ loading, disabled: isDisabled, className, ...props }: CustomSwitchProps) {
  return (
    <SwitchPrimitive.Root
      ...
      <SwitchPrimitive.Thumb
        ...
        className={cn(
          ...
          // TODO: use linear color
          isDisabled && 'bg-primary backdrop-grayscale-50'
        )}>
        {loading && <LoaderIcon className="animate-spin" />}
        {!loading && <div className="w-0.5 h-1.5"></div>}
      </SwitchPrimitive.Thumb>
    </SwitchPrimitive.Root>
  )
}
